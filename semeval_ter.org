...
Les approches bruteforce correspondent à tester toutes les combinaisons d'arbres de documents, de sacs de mots et de filtres.
...
Le but des approches de pondération est de d'utiliser plusieurs indicateurs au sein d'une même méthode.
...
*** Création des approches de pondération
...
#+BEGIN_SRC ipython :exports results :results drawer output :tangle no :session semexec
  ...
  def makerestable(doctree):
      resulttable=[]
      for indicator, filterspartition in product(all_indicators, filters_partition):
          predfilename = getpredfilename(
              doctree, indicator, filterspartition)
