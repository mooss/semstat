...
:PROPERTIES:
:header-args: :tangle semeval_executable.py :exports code :session semexec :eval no-export
:END:
** Imports
...
*** Méthodes bruteforce
**** Création des méthodes bruteforce
...
**** Exécution des méthodes bruteforce
...
#+BEGIN_SRC ipython :tangle no :exports results :results output drawer replace
  ...
  for doctree in doctrees:
      restable = []
      for indicator, fltrs in product(*ponderated_methods[1:]):

          predfilename = getpredfilename(
              doctree, indicator, fltrs, 'nerponderation')

          score = subprocess.run(
              ['./extractMAP.sh', predfilename], stdout=subprocess.PIPE)

          line = [all_indicators_descr[indicator],
                  ', '.join(filters_descr[key] for key in fltrs),
                  score.stdout.decode('utf-8').strip('\n')]

          restable.append(line)
...
#+BEGIN_SRC ipython :exports results :results drawer output :tangle no :session semexec
  ...
  def makerestable(doctree):
      resulttable=[]
      for indicator, filterspartition in product(*bruteforce_methods[1:]):
          predfilename = getpredfilename(
              doctree, indicator, filterspartition, 'bruteforce')
